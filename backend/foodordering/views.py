...
def list_category(request):
    ...
    serializer = categortSerializer(categories,many=True)
...
def user_register(request):
    ...
    if User.objects.filter(email  = email).exists() or User.objects.filter(mobile = mobile_no).exists():
        return Response(
        {"message": "Email or Mobile Number already registered"},
        status=400
    )
    User.objects.create(first_name =  firstname,last_name=lastname,email=email, mobile=mobile_no,
...
from rest_framework.response import Response
from .models import Order, OrderAddress, PaymentDetail, User
...
def place_order(request):
    ...
    try:
        ...
        # ✅ Validate cart orders
        order = Order.objects.filter(user=user, is_order_placed=False)
        if not order.exists():
            return Response({"message": "No active cart items found"}, status=400)

        # ✅ Generate unique order number
        order_number = make_unique_order_number()

        # ✅ Update all cart items to 'placed'
...
@api_view(['GET'])
def order_byorder_number(request,order_number):
...
def order_between_dates(request):
    ...
    if from_date_str and to_date_str:
        ...
        orders = orders.filter(order_time__range=[from_date, to_date])

    if status == "not_confirmed":
        orders = orders.filter(Q(order_final_status__isnull=True) | Q(order_final_status=""))
    elif status != "all":
        orders = orders.filter(order_final_status=status)

    print("From:", from_date_str, "To:", to_date_str, "Status:", status)
...
def search_orders(request):
    ...
    if query:
        ...
        orders = []  # ✅ same variable name

    serializer = OrderSummarySerializer(orders, many=True)  # ✅ correct variable
    return Response(serializer.data)
...
def category_detail(request, id):
    ...
    if request.method == 'GET':
        serializer = categortSerializer(category)
        return Response(serializer.data)

    elif request.method == 'PUT':
        serializer = categortSerializer(category, data=request.data)
        if serializer.is_valid():
            serializer.save()
...
def edit_food(request, id):
    ...
    if request.method == 'GET':
        ...
        if 'is_available' in data:
            data['is_available'] = str(data['is_available']).lower() in ['true', '1']

        serializer = FoodSerializer(food, data=data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=200)  # ✅ Return full updated object
        return Response(serializer.errors, status=400)
...
def dashboard_metrics(request):
    ...
    # Get all paid orders on or after start date
      paid_orders = PaymentDetail.objects.filter(payment_date__gte=aware_start
    ).values_list('order_number', flat=True)

    # Calculate total from related orders
      total = (Order.objects.filter(order_number__in=paid_orders)
        .annotate(total_price=F('quantity') * F('food__item_price'))  # ✅ fixed line
